...
const processors = {
  ...
  compileSource: (type, buildInfo) => {
    ...
    // 编译 - 将SCSS编译为CSS
    if (type === 'css') {
      const result = sass.compileString(body, { style: 'expanded', charset: false });
  ...
  compressContent: async (content, type) => {
    if (type === 'css') {
      return new CleanCSS({ level: { 1: { all: true }, 2: { all: true } } }).minify(content).styles;
    }
    // JS 类型
...
const buildAll = async (buildInfo, isDevBuild = false) => {
  ...
  if (failed.length > 0) {
    ...
    failed.forEach(item => item.error && console.error(`   ${item.type.toUpperCase()}错误:`, item.error.message));
    return false;
  } else {
    console.log(`\n✅ 构建成功! 总耗时: ${totalBuildTime}ms`);
    if (successful.length > 0) {
      console.log(`📁 输出文件: ${successful.map(item => path.basename(CONFIG[item.type][isDevBuild ? 'devFile' : 'prodFile'])).join(', ')}`);
    }
    return true;
  }
};
